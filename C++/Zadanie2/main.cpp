...
List* copyListReverse(List* list) {
    List *list_novy = new List;
	list_novy->first = nullptr;
	
	Node* tmp= list->first;
	while(tmp){
		prependNode(list_novy,tmp->data);
		tmp = tmp->next;
	}
	return list_novy; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
int main(int argc, char** argv) {
	/*
	createListFromArray(data,lenght_array);
	createSymmetricList(6);
	removeFirstNode(list);
	findNodeInList(list,11);
	areListsEqual(list,list2);
	
	*/
    int data[] = {1,5,4};
	int lenght_array = sizeof(data)/sizeof(int);
	List *list = new List;
	list->first = nullptr;
 	//list = createListFromArray(data,lenght_array);
	list = createSymmetricList(5);
	printList(list);
    return 0;
}
